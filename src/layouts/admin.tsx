...
const NestedNavMenu = ({
    ...
    return (
        <div className="cursor-pointer">
            <div className="group flex items-center px-2 py-2.5 rounded hover:bg-blue-50 transition-colors duration-300"
                 aria-selected={selected}
                 onClick={() => setOpended(!opened)}>
                <span>{icon}</span>
                <span
                    className="flex-1 ml-3 whitespace-nowrap group-hover:text-blue-600 group-aria-selected:text-blue-600 transition-colors duration-300">
                    {label}
                </span>
                ...
                <div className="w-full overflow-hidden">
                    {items.map(item => (
                        <Link to={item.href}
                              key={item.href}
                              data-key={item.href}
                              className="oa-nav-item mt-2 block py-2.5 pl-10 whitespace-nowrap rounded text-gray-600 hover:bg-blue-50 hover:text-blue-600 aria-selected:text-blue-600 aria-selected:bg-blue-100 transition-colors duration-300">
                            {item.label}
                        </Link>
                    ))}
                </div>
...
const NavMenuItem = ({
    ...
    return (
        <Link to={to}
              data-key={datakey}
              className="oa-nav-item group flex items-center px-2 py-2.5 rounded hover:bg-blue-50 aria-selected:bg-blue-100 transition-colors duration-300">
            {icon}
            <span
                className="ml-3 group-hover:text-blue-600 group-aria-selected:text-blue-600 transition-colors duration-300">
                {text}
            </span>
        </Link>
    )
...
const NavMenu = () => {
    ...
    useEffect(() => {
        const items = document.getElementsByClassName('oa-nav-item')
        ...
        <ul className="space-y-2 text-sm text-gray-900 dark:text-white select-none">
                             ...
                             icon={(
                                 <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                                     ...
                                     <path d="M2 10a8 8 0 018-8v8h8a8 8 0 11-16 0z"
                                           className="fill-gray-400 group-hover:fill-blue-600 group-aria-selected:fill-blue-600 transition-colors duration-300"></path>
                                     <path d="M12 2.252A8.014 8.014 0 0117.748 8H12V2.252z"
                                           className="fill-gray-300 group-hover:fill-gray-400 group-aria-selected:fill-gray-400 transition-colors duration-300"></path>
                                 </svg>
                             )}/>
            </li>
            <li>
                <NestedNavMenu
                    ...
                    icon={(
                        <Network
                            className="w-5 h-5 stroke-gray-500 group-aria-selected:stroke-blue-600 group-hover:stroke-blue-600"/>
                    )}
                    label="组织机构"
                    items={[
            ...
            <li>
                <NestedNavMenu
                    ...
                    icon={(
                        <Key
                            className="w-5 h-5 stroke-gray-600 group-aria-selected:stroke-blue-600 group-hover:stroke-blue-600"/>
                    )}
                    label="权限管理"
                    items={[
    ...
    return (
        <div className="p-2 h-14 flex justify-between items-center">
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent
                    className="w-56 p-2 text-xs rounded-lg text-gray-600 hover:text-gray-800 dark:text-gray-200"
                                ...
                                <div className="p-4">
                                    <div className="grid grid-cols-2 gap-x-4 text-xs text-gray-500">
                                        <div className="flex flex-col gap-y-2 items-center cursor-pointer"
                                            ...
                                            <span
                                                className={classNames("rounded-full px-2 py-1", notDark && "bg-blue-600 text-white")}>
                                                浅色
                                            </span>
                                        </div>
                                        <div className="flex flex-col gap-y-2 items-center cursor-pointer"
...
const AdminLayout = () => {
    ...
    return (
        <div className="h-screen min-w-[1280px] flex bg-background text-gray-800 dark:text-white dark:bg-slate-900">
            <aside
                className="flex flex-col flex-initial w-[240px] min-w-[240px] h-full overflow-hidden relative bg-slate-50 dark:bg-slate-800 dark:text-white">
                <Link to="/admin">
